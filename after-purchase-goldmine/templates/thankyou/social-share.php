 ...
 *
 * @var \WC_Order $order
 * @var array[]   $networks   keyed by network => [label, href]
...
$apg_icons = array(
	'x'        => '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2H21.5l-7.5 8.57L23 22h-6.594l-5.165-6.75L5.34 22H2.08l8.02-9.166L1 2h6.76l4.67 6.17L18.244 2Zm-1.157 18h1.832L7.01 3.88H5.04L17.087 20Z"/></svg>',
	...
	'linkedin' => '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M4.98 3.5A2.5 2.5 0 1 0 5 8.5a2.5 2.5 0 0 0-.02-5ZM3 9h4v12H3zM9 9h3.8v1.64h.05c.53-1 1.83-2.05 3.77-2.05 4.03 0 4.78 2.65 4.78 6.1V21h-4v-5.4c0-1.29-.02-2.95-1.8-2.95-1.8 0-2.08 1.4-2.08 2.85V21H9z"/></svg>',
...
?>
<article class="apg-card apg-card--share" data-module="social_share">
	...
	<div class="apg-share__grid">
		<?php foreach ( $networks as $key => $net ) : ?>
			<?php if ( 'copy' === $key ) : ?>
				<button type="button" class="apg-share-btn apg-share-btn--<?php echo esc_attr( $key ); ?> apg-js-copy apg-js-track" data-module="social_share" data-clipboard="<?php echo esc_url( $share_url ); ?>" aria-label="<?php echo esc_attr( $net['label'] ); ?>">
					<?php echo isset( $apg_icons[ $key ] ) ? $apg_icons[ $key ] : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<span><?php echo esc_html( $net['label'] ); ?></span>
				</button>
			<?php else : ?>
				<a class="apg-share-btn apg-share-btn--<?php echo esc_attr( $key ); ?> apg-js-track" data-module="social_share" target="_blank" rel="noopener nofollow" href="<?php echo esc_url( $net['href'] ); ?>" aria-label="<?php echo esc_attr( $net['label'] ); ?>">
					<?php echo isset( $apg_icons[ $key ] ) ? $apg_icons[ $key ] : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<span><?php echo esc_html( $net['label'] ); ?></span>
				</a>
			<?php endif; ?>
